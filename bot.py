...
async def profile_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        my_message = await send_text(update, context, " 🧠 ChatGPT генерирует ваш профиль...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["age"] = text
        await send_text(update, context, "Оцените внешность: 1-10 баллов?")
    elif dialog.count == 3:
...
app = ApplicationBuilder().token("7273308033:AAGHFFjCPb3qRw9WNtnOv7UrA56kSu3BASM").build()
